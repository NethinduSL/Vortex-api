    ...
    switch (type) {
      ...
      case 'rps':
        ...
        if (state.pendingRPS[players[0]] && state.pendingRPS[players[1]]) {
          ...
          if (result === null) {
            // Tie
            state.moves.push(`⚖️ Tie! Both chose ${player1Choice}. Replaying RPS.`);
            state.pendingRPS = {};
        ...
        switch (action) {
          ...
          case 'shield':
            if (state.islands[username].shields < 3) {
              ...
              state.moves.push(`🛡️ ${username} bought Shield! Shields increased to ${state.islands[username].shields}`);
            } else {
              state.moves.push(`🛡️ ${username} tried to buy Shield but already at maximum (3)`);
            }
            break;
